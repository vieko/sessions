...
> 2. Run: `claude --append-system-prompt "$(cat .bonfire/handoff/context.md)"`
...
## Step 9: Spawn New Claude Session
...
tmux split-window -h -c "$REPO_ROOT" \
  "claude --append-system-prompt '$CONTEXT' 'Continuing from session handoff. Please run /bonfire:start to load the full session context.'"
```
